...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--time", required=True, nargs="+", help='時刻 (複数可)。例: "9:00" "10:00"')
    p.add_argument("--menu", required=True, help='メニュー名 (部分一致)。例: "技能教習"')
...
def main() -> None:
    ...
    target_date = args.date or (datetime.now() + timedelta(days=2)).strftime("%Y/%m/%d")
    ...
    for target_time in args.time:
        ...
        detail_url = find_reservation_url(calendar_html, target_date, target_time, args.menu)
